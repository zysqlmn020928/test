...
public class ItemController {
    ...
    public ApiResult ItemController(@RequestBody Item item) {
        ...
        for (Integer questionId : changeNumbers) {
            PaperManage paperManage = new PaperManage(paperId,1,questionId);
        ...
        for (Integer questionId : fills) {
            PaperManage paperManage = new PaperManage(paperId,2,questionId);
            int index = paperService.add(paperManage);
            if(index==0){
        ...
        for (Integer questionId : judges) {
            PaperManage paperManage = new PaperManage(paperId,3,questionId);
            int index = paperService.add(paperManage);
            if(index==0){
